import 'package:flutter/material.dart';

class AppColors {
  static const Color fitnessMainColor = Color(0xFF48CC6D);
  static const Color fitnessModuleColor = Color(0xFF191B1C);
  static const Color fitnessSecondaryModuleColor = Color(0xFF2B2B2B);
  static const Color fitnessBackgroundColor = Color(0xFF000000);
  static const Color fitnessPrimaryTextColor = Colors.white;
  static const Color fitnessSecondaryTextColor = Color(0xFF787878);
  static const Color fitnessWarningColor = Color(0xFFAD3838);
}