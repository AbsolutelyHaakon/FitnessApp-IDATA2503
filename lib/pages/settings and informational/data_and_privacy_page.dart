...
class _DataAndPrivacyPageState extends State<DataAndPrivacyPage> with SingleTickerProviderStateMixin {
  ...
  bool _viewDataPressed = false; // Flag to check if "View Your Data" button is pressed
  ...
  final WorkoutDao _workoutDao = WorkoutDao(); // DAO for workout data
  ...
  Future<void> _fetchData() async {
    if (FirebaseAuth.instance.currentUser != null) {
      ...
      await _fetchWorkoutData(); // Fetch workout data
    }
  }
  ...
  Widget build(BuildContext context) {
    return DefaultTabController(
      ...
      child: Scaffold(
        appBar: AppBar(
          backgroundColor: AppColors.fitnessBackgroundColor, // Background color of app bar
          title: const Text('Data and Privacy',
              style: TextStyle(color: AppColors.fitnessPrimaryTextColor)), // Title of app bar
          leading: IconButton(
            icon: const Icon(CupertinoIcons.back,
                color: AppColors.fitnessMainColor), // Back button
            onPressed: () => Navigator.of(context).pop(), // Go back to previous page
          ),
          bottom: TabBar(
            controller: _tabController, // Tab controller
            indicator: const BoxDecoration(), // Tab indicator
            labelColor: AppColors.fitnessMainColor, // Color of selected tab label
            unselectedLabelColor: AppColors.fitnessSecondaryTextColor, // Color of unselected tab label
            labelStyle: const TextStyle(fontWeight: FontWeight.w900, fontSize: 16), // Style of selected tab label
            unselectedLabelStyle: const TextStyle(fontWeight: FontWeight.normal, fontSize: 16), // Style of unselected tab label
            tabs: const [
        ...
        backgroundColor: AppColors.fitnessBackgroundColor, // Background color of scaffold
      ),
    );
  }
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 20), // Space between elements
          ListTile(
            leading: Icon(Icons.visibility, color: AppColors.fitnessMainColor), // Icon for the list tile
            title: const Text('View Your Data',
                style: TextStyle(
                    color: AppColors.fitnessPrimaryTextColor, fontSize: 18)), // Title of the list tile
            ...
            child: Row(
              ...
              children: [
                ...
                Switch(
                    ...
                    if (!value) {
                      bool confirmChange = await showDialog(
                        ...
                        builder: (context) => AlertDialog(
                          backgroundColor: AppColors.fitnessModuleColor, // Background color of dialog
                          ...
                          actions: [
                            TextButton(
                              onPressed: () => Navigator.of(context).pop(false), // Close dialog without changing
                              child: const Text(
                                'No',
                                style: TextStyle(
                            ...
                            TextButton(
                              onPressed: () => Navigator.of(context).pop(true), // Close dialog and change setting
                              child: const Text(
                                'Yes',
                                style: TextStyle(
                                    color: AppColors.fitnessWarningColor), // Text color
                              ),
                            ),
                          ],
                        ),
                  ...
                  activeColor: AppColors.fitnessMainColor, // Active color of switch
                ),
              ],
            ),
          ),
          const SizedBox(height: 20), // Space between elements
          ListTile(
            leading: Icon(Icons.delete, color: AppColors.fitnessWarningColor), // Icon for the list tile
            title: const Text('Delete My Data',
                style: TextStyle(
                    color: AppColors.fitnessWarningColor, fontSize: 18)), // Title of the list tile
            trailing: Transform.scale(
              scale: 0.6, // Scale down the icon
              child: const Icon(CupertinoIcons.right_chevron,
  ...
  Widget _buildDataTab() {
    ...
    return Padding(
      ...
      child: ListView(
        children: [
          _buildDataSection('USERDATA', _allData['userdata']), // Build user data section
          _buildDataSection('WORKOUTS', _allData['workouts']), // Build workouts data section
          _buildDataSection('EXERCISES', _allData['exercises']), // Build exercises data section
          _buildDataSection('USERWORKOUTS', _allData['userworkouts']), // Build user workouts data section
          _buildDataSection('POSTS', _allData['posts']), // Build posts data section
          _buildDataSection('USERFOLLOWS', _allData['userfollows']), // Build user follows data section
        ],
      ),
    );
  }
  ...
  Widget _buildDataSection(String title, dynamic data) {
    return Container(
      padding: const EdgeInsets.all(16.0), // Padding inside the container
      margin: const EdgeInsets.only(bottom: 20.0), // Margin around the container
        ...
        children: [
          ...
          if (data is Map<String, dynamic>)
            ...data.entries.map((entry) => Padding(
                  padding: const EdgeInsets.only(bottom: 8.0), // Padding around the entry
                  child: RichText(
                    text: TextSpan(
                      children: [
                        TextSpan(
  ...
  void _deleteMyData() async {
    bool confirmDelete = await showDialog(
      ...
      builder: (context) => AlertDialog(
        backgroundColor: AppColors.fitnessModuleColor, // Background color of dialog
        title: const Text('Delete My Data',
            style: TextStyle(color: AppColors.fitnessWarningColor)), // Title of dialog
        content: RichText(
          text: const TextSpan(
            children: [
              TextSpan(
                ...
                style: TextStyle(color: AppColors.fitnessPrimaryTextColor, fontSize: 15), // Text style
              ),
              TextSpan(
                text: 'This action cannot be undone.',
                style: TextStyle(color: AppColors.fitnessWarningColor, fontSize: 15, fontWeight: FontWeight.bold), // Text style
              ),
            ],
          ),
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.of(context).pop(false), // Close dialog without deleting
            child: const Text('Cancel',
                style: TextStyle(color: AppColors.fitnessMainColor)), // Text style
          ),
          TextButton(
            onPressed: () => Navigator.of(context).pop(true), // Close dialog and delete data
            child: const Text('Delete',
                style: TextStyle(color: AppColors.fitnessWarningColor)), // Text style
          ),
        ],
      ),
    );
